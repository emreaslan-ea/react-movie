            ...
            {this.props.movies.map((movie) => (
              ...
              <div href="#" className="group drop-shadow-md" key={movie.id}>
                <div className="px-0.5 w-full overflow-hidden">
                  <img src={movie.imageURL} alt="Movie poster" className="h-full w-full object-cover object-center group-hover:opacity-75 rounded-t-lg" />
                </div>
                <div className='p-3 pt-1 -mt-1 border-solid border-slate-200 border-2 rounded-b-lg'>
                  <h3 className="mt-2 text-lg text-gray-900">{movie.name}</h3>
                  ...
                  <div className='flex justify-between'>
                    ...
                    <span className='p-1 px-3 mt-3 bg-sky-500 text-lg text-white rounded-lg'>{movie.rating}</span>
                  </div>
                </div>
              </div>
